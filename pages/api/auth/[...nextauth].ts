...
import { NextApiRequest, NextApiResponse } from "next"; // NextApiRequest, NextApiResponse 타입 추가
...
const authOptions = {
    ...
    session: {
        strategy: "jwt" as const, // Next.js 13+에서는 'jwt'를 const로 설정
        maxAge: 30 * 24 * 60 * 60,
    },
    callbacks: {
...
export default async function authHandler(
    ...
    // ⭐️⭐️⭐️ NextAuth 핸들러 진입 시 ALB 헤더 디버그 로그 추가 (핵심) ⭐️⭐️⭐️
    console.log("--- [DEBUG] ALB Header Check (NextAuth Handler Start) ---");
    console.log("Method:", req.method);
    console.log("URL:", req.url);
    console.log(
        "X-Forwarded-Proto:",
        req.headers["x-forwarded-proto"] || "헤더 없음"
    );
    console.log("Host:", req.headers.host || "헤더 없음");
    console.log("-----------------------------------------------------");
