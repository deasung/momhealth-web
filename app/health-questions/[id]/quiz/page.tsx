...
const QuizPage = () => {
  ...
  // ✅ 성능: 계산값 메모이제이션 (메모리 최적화)
  const items = useMemo(() => quiz?.items || [], [quiz?.items]);
  const currentItem = useMemo(() => {
    if (!items.length || current < 0 || current >= items.length) return null;
    return items[current];
  }, [items, current]);
  const selectedChoiceId = useMemo(() => {
    if (!currentItem?.id) return undefined;
    return answers.find((a) => a.itemId === currentItem.id)?.choiceId;
  }, [answers, currentItem?.id]);
  const isLastQuestion = useMemo(
    () => items.length > 0 && current === items.length - 1,
    [current, items.length]
  );
  const progress = useMemo(
    () => (items.length > 0 ? ((current + 1) / items.length) * 100 : 0),
    [current, items.length]
  );
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between gap-2 sm:gap-4">
            ...
            {/* ✅ 디자인: 진행률 배지 개선 */}
            <div className="bg-gradient-to-br from-orange-100 to-orange-50 border border-orange-200 rounded-lg px-3 sm:px-4 py-1.5 shadow-sm">
